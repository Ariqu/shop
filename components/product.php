<section class="bg-white dark:bg-gray-900">
    <div class="container flex flex-col items-center px-4 py-12 mx-auto xl:flex-row">
        <div class="flex justify-center xl:w-1/2">
            <img class="h-80 w-80 sm:w-[28rem] sm:h-[28rem] flex-shrink-0 object-cover rounded-full" src="zdjecia/d1.jpg" alt="">
        </div>

        <div class="flex flex-col items-center mt-6 xl:items-start xl:w-1/2 xl:mt-0">
            <h2 class="text-2xl font-semibold tracking-tight text-gray-800 xl:text-3xl dark:text-white">
                Najpopularniejszy produkt w tym miesiącu
            </h2>

            <p class="block max-w-2xl mt-4 text-gray-500 dark:text-gray-300">Ta ikoniczna kompozycja zapachowa otwiera się świeżymi nutami bergamotki, tworząc intensywny i energetyczny wstęp. W sercu zapachu dominuje mieszanka paczuli z esencją szałwii, nadając mu wyrazisty, zmysłowy charakter. Całość otula ciepła baza składająca się z nut ambroxanu, co nadaje zapachowi głębi i trwałości.</p>

            <div class="mt-6 sm:-mx-2">
                <a href="#" class="inline-flex items-center justify-center w-full px-4 text-sm py-2.5 overflow-hidden text-white transition-colors duration-300 bg-gray-900 rounded-lg shadow sm:w-auto sm:mx-2 hover:bg-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 focus:ring focus:ring-gray-300 focus:ring-opacity-80">
                    

                    <span class="mx-2">
                        Zobacz produkt
                    </span>
                </a>

            </div>
        </div>
    </div>
</section>